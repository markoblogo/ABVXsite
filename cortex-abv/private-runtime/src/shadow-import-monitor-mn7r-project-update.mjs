...
import { createMonitorMn7rShadowImport } from './monitor-mn7r-shadow-import.mjs';
...
export function run() {
  ...
  const packet = createMonitorMn7rShadowImport({ projectUpdate });
  ...
  console.log(JSON.stringify({
    ...
    productId: projectUpdate.productId,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    console.error(`CortexABV Monitor/MN7R shadow import failed: ${error.message}`);
    process.exitCode = 1;
  }
}
