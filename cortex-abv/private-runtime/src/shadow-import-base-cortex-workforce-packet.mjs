...
import { createBaseCortexShadowImport } from './base-cortex-shadow-import.mjs';
...
export function run() {
  ...
  const workforcePacket = JSON.parse(readFileSync(sourcePacketPath, 'utf8'));
  const packet = createBaseCortexShadowImport({ workforcePacket });
  ...
  console.log(JSON.stringify({
    ...
    taskId: workforcePacket.taskId,
    correlationId: workforcePacket.correlationId,
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    console.error(`CortexABV base Cortex shadow import failed: ${error.message}`);
    process.exitCode = 1;
  }
}
